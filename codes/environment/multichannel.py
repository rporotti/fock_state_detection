...
class SimpleCavityEnv(gym.Env):
    ...
    def init_var(self,args):
        ...
        self.multiplier=dic["multiplier"]
...
#             self.summary.close()
    ...
    def step(self,action):
        ...
        for step in range(self.numberPhysicsMicroSteps):
            ...
            if self.kappa>0:
                ...
                k2=self.dt*self.kappa*( np.matmul(self.aOp,np.matmul(self.Rho+k1/2,self.adOp)) -
                        0.5*(np.matmul(self.adaOp,self.Rho+k1/2)+np.matmul(self.Rho+k1/2,self.adaOp) ))
                k3=self.dt*self.kappa*( np.matmul(self.aOp,np.matmul(self.Rho+k2/2,self.adOp)) -
                        0.5*(np.matmul(self.adaOp,self.Rho+k2/2)+np.matmul(self.Rho+k2/2,self.adaOp) ))
            ...
            if self.kappa_meas>0:
                ...
                k1=np.sqrt(self.kappa_meas/2)*dW[:,np.newaxis,np.newaxis]*(temp - quadrature[:,np.newaxis,np.newaxis]*self.Rho )
                k2=np.sqrt(self.kappa_meas/2)*dW[:,np.newaxis,np.newaxis]*(temp - quadrature[:,np.newaxis,np.newaxis]*self.Rho+k1/2 )
                k3=np.sqrt(self.kappa_meas/2)*dW[:,np.newaxis,np.newaxis]*(temp - quadrature[:,np.newaxis,np.newaxis]*self.Rho+k2/2 )
                k4=np.sqrt(self.kappa_meas/2)*dW[:,np.newaxis,np.newaxis]*(temp - quadrature[:,np.newaxis,np.newaxis]*self.Rho+k3 )
            ...
            self.probabilities[:,int(self.t*self.numberPhysicsMicroSteps+step)]=np.real(np.diag( self.Rho))
        ...
        self.observations[self.t]=self.X # add the new msmt result
        ...
        self.integrals[self.t-1]=1/(self.t*self.dt)*(np.trapz(self.observations[:self.t-1],axis=0,dx=self.dt))
    ...
    def evaluate_policy(self):
        if self.rank==0 and self.counter==0:
            ...
            if self.viewer==True and self.ep%self.save_every==0:
                ...
                print("Last "+str(self.save_every)+" episodes, av. reward="+str(np.mean(self.total_rewards[-self.save_every:])) )
                print("Last "+str(self.save_every)+" episodes, prob. success="+str(np.mean(self.total_rewards[-self.save_every:])) )
                self.render()
    ...
    def create_figure(self):
        ...
        if self.testing:
            ...
            gs = self.figure.add_gridspec(5, 4)
            self.axes=np.zeros((3,4),dtype="object")
            self.axes_integral=np.zeros((3,4),dtype="object")
        ...
        if self.testing:
            ...
            offset=1
            self.axes[0,:]=self.figure.add_subplot(gs[1+offset, :3])
            self.ax_histo2=self.figure.add_subplot(gs[1+offset, -1])
        ...
        for count in range(self.N):
            i=int(count/4)
            j=count%4
            self.axes[-2+i,j]=self.figure.add_subplot(gs[-2+i, j])
            if i==0:
                plt.setp(self.axes[-2+i,j].get_xticklabels(), visible=False)
            if i==1:
                self.axes[-2+i,j].set_ylabel(r"$t/\Delta t$")
        #ax4 = fig.add_subplot(gs[1:, -1])
        #self.axes[self.offset,0].set_ylim(0,self.Nstates);
        #self.axes[-2,1].set_ylim(0,1.01)
        #self.axes[-2,2].set_ylim(0,1.01);

        for i in range(2):
            for j in range(4):
                self.axes[i,j].set_xlim(0,self.T)
                #self.axes[offset,j].set_ylim(self.lim_actions[i][0]-0.1,self.lim_actions[i][1]+0.1);
    ...
    def render(self,mode='rgb_array',close=True):
        ...
        for count in range(self.N):
            ...
            maxim=np.max(np.abs(self.observations[:,:,count]))
